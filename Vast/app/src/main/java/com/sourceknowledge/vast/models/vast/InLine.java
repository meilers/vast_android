...
public class InLine implements Parcelable {
    ...
    public static Creator<InLine> CREATOR = new Creator<InLine>() {

        @Override
        public InLine createFromParcel(Parcel in) {
            return new InLine(in);
        }

        @Override
        public InLine[] newArray(int size) {
            return new InLine[size];
        }
    };
